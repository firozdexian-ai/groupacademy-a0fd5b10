...
Deno.serve(async (req) => {
  ...
  // 1. Upsert to suppressed_emails (idempotent â€” safe for retries)
  ...
  if (insertError) {
    // Non-fatal â€” log and continue. The suppression was already recorded.
...
function mapReasonToMessage(reason: string): string {
  switch (reason) {
    case 'bounce':
      return 'Permanent bounce â€” email address is invalid or rejected'
    case 'complaint':
      return 'Spam complaint â€” recipient marked email as spam'
    case 'unsubscribe':
