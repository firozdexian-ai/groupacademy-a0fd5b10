...
serve(async (req) => {
  ...
  try {
    // 1. SECURITY: Verify the User via JWT claims (no session needed)
    ...
    // Get session ID - create if not provided, verify if provided
    let actualSessionId = sessionId;

    if (sessionId) {
      // 2. SECURITY: Verify Ownership of existing session
      ...
      if (sessionError) {
        ...
        return new Response(JSON.stringify({ error: "Failed to create lead hunt session" }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    // Fetch talents with CV data (Using Admin Client to search all talents)
    // Note: In a real app, you might want to filter this further or use Full Text Search
    ...
    if (talentsError) {
      ...
      return new Response(JSON.stringify({ error: "Failed to fetch talents" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (matchInserts.length > 0) {
      const { error: insertError } = await supabaseAdmin.from("lead_hunt_matches").upsert(matchInserts, {
...
function calculateInitialScore(talent: any, requirements: any): number {
  ...
  if (requirements.educationKeywords.length > 0) {
    const hasRequiredEdu = requirements.educationKeywords.some((edu: string) => talentText.includes(edu));
