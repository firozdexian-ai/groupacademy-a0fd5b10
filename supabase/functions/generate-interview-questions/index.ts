...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Parse Request
    const { jobDescription, questionCount, difficulty, professionCategoryId, additionalNotes, candidateProfile } =
      await req.json();
    ...
    // Initialize Admin Client for Data Fetching
    ...
    const difficultyGuide = {
      ...
      medium:
        "Ask moderately challenging questions suitable for mid-level candidates. Include behavioral and situational questions.",
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "questions": [
    ...
    console.log(`Generating interview questions for user: ${user.id}`);
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert HR interviewer. Generate professional interview questions based on job descriptions. Always respond with valid JSON only.",
          },
