...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 2) Cold-start: unknown active gig with ZERO matches yet (no time filter)
    const { data: matched } = await supabase
      .from("gig_matches")
      .select("gig_id");
    const matchedIds = new Set((matched ?? []).map((m: unknown) => m.gig_id));
    ...
    const cold = (allActive ?? []).filter((g: unknown) => !matchedIds.has(g.id));

    // Merge & dedupe
    const map = new Map<string, { id: string; kind: string }>();
    for (const g of [...(recent ?? []), ...cold]) {
      map.set((g as unknown).id, { id: (g as unknown).id, kind: (g as unknown).kind });
    }
    ...
    for (let i = 0; i < queue.length; i += BATCH_SIZE) {
      ...
      batch.forEach((b, idx) => {
        if (b.status === "fulfilled") {
          results.push({ gig_id: slice[idx].id, kind: slice[idx].kind, matched: (b.value as unknown)?.data });
        } else {
