...
export async function getHrOnboardingMaster() {
  ...
  return {
    ...
    workforce: (workforceRes.data ?? []) as unknown[],
  };
}
...
export async function getHrPayrollMaster() {
  ...
  if (workforceRes.error) throw workforceRes.error;
  return {
    runs: (payrollRes.data ?? []) as unknown[],
    workforce: (workforceRes.data ?? []) as unknown[],
  };
}
...
export async function insertWorkforceMember(payload: Record<string, unknown>): Promise<void> {
  const { error } = await supabase.from("workforce_members").insert(payload as unknown);
  if (error) throw error;
}
...
export async function deleteWorkforceRoutingRule(id: string): Promise<void> {
  const { error } = await (supabase as unknown).from("workforce_routing_rules").delete().eq("id", id);
  if (error) throw error;
}
...
export async function deleteWorkforceChannelConnection(id: string): Promise<void> {
  const { error } = await (supabase as unknown).from("workforce_channel_connections").delete().eq("id", id);
  if (error) throw error;
}
...
export async function upsertWorkforceChannelConnection(
  ...
  const q = (supabase as unknown).from("workforce_channel_connections");
  const { error } = id ? await q.update(payload).eq("id", id) : await q.insert(payload);
  if (error) throw error;
}

export async function listWorkforceRoutingRules(): Promise<unknown[]> {
  const { data, error } = await (supabase as unknown)
    .from("workforce_routing_rules")
    .select("*")
    .order("event_topic");
...
export async function upsertWorkforceRoutingRule(
  ...
  const q = (supabase as unknown).from("workforce_routing_rules");
  const { error } = id ? await q.update(payload).eq("id", id) : await q.insert(payload);
  if (error) throw error;
}
...
export async function listActiveWorkforceMembersWithName(): Promise<unknown[]> {
  const { data, error } = await (supabase as unknown)
    .from("workforce_members")
...
export async function upsertHrTarget(payload: Record<string, unknown>): Promise<void> {
  if (payload?.id) {
    const { id, ...patch } = payload;
    const { error } = await (supabase as unknown).from("hr_targets").update(patch).eq("id", id);
    if (error) throw error;
  } else {
    const { error } = await (supabase as unknown).from("hr_targets").insert(payload);
    if (error) throw error;
  }
}

export async function deleteHrTarget(id: string): Promise<void> {
  const { error } = await (supabase as unknown).from("hr_targets").delete().eq("id", id);
  if (error) throw error;
}
 ...
 */
export async function getWorkforceMembersForCompany(companyId: string): Promise<unknown[]> {
  const { data, error } = await (supabase as unknown)
    .from("workforce_members")
    .select(
 ...
 */
export async function getMyWorkforceAssignment(talentId: string): Promise<unknown | null> {
  const { data, error } = await (supabase as unknown)
    .from("workforce_members")
    .select(
