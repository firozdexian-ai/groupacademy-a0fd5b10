    ...
    <PublicLayout>
      <main className="relative pt-20 pb-32 overflow-hidden selection:bg-primary/10">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          {/* Skeletons or Agents Grid */}
          {isLoading ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto items-stretch animate-pulse">
              {[1, 2, 3].map((idx) => (
                <Card key={idx} className="rounded-[32px] border-border/40 bg-card/45 h-96" />
              ))}
            </div>
          ) : (
                    ...
                    <div>
                      <CardHeader className="p-8 pb-4">
                        <div className="flex items-center gap-4">
                          <div className={`p-4 rounded-2xl ${agent.bg_color || "bg-muted"} ${agent.color || "text-foreground"} group-hover:scale-110 transition-transform duration-300`}>
          ...
          <div className="text-center mt-20 p-8 md:p-12 rounded-[40px] border border-border/40 bg-card/30 max-w-4xl mx-auto backdrop-blur-sm space-y-6">
            ...
            <p className="text-xs text-muted-foreground font-medium max-w-lg mx-auto leading-relaxed">
              Create your account now to claim your welcome credits and begin a chat session with one of our specialized AI coaches immediately.
            </p>
