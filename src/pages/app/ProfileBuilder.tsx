...
export default function ProfileBuilder() {
  const { talent, refreshTalent, updateTalent } = useTalent();
    ...
    try {
      ...
      if (data.handoff?.agent_key && data.handoff.agent_key !== coach?.agent_key) {
        ...
        if (agentRow) {
          ...
          await updateTalent({ onboardingCompletedAt: new Date().toISOString() });
        }
      }
  ...
  }, [input, messages, sending, coach, refreshTalent, updateTalent]);

  const readiness = computeReadiness(talent);
  
  const displayName = coach?.name 
    ? coach.name 
    : (talent?.institution ? `${talent.institution} Representative` : "Aisha");

  const displaySubtitle = coach 
    ? "Domain Coach" 
    : (talent?.institution ? "University Representative" : "Profile Concierge");
