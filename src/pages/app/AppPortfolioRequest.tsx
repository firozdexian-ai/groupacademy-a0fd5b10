...
export default function AppPortfolioRequest() {
 ...
 return;
  ...
  if (!isFreePromotionActive && !canAfford("PORTFOLIO")) {
    toast({
      ...
      description: `Building a portfolio requires ${PORTFOLIO_COST.toString()} credits.`,
      variant: "destructive",
    });
    return;
  }
 ...
 try {
      if (!isFreePromotionActive) {
        const isPaymentSettled = await deductCredits("PORTFOLIO", undefined, "Custom Portfolio Building Service");
        if (!isPaymentSettled) throw new Error("Deduction failed. Please try again.");
      }
 ...
 <Card className="rounded-xl border border-border/60 bg-card shadow-none overflow-hidden block w-full">
 ...
 <CardTitle className="text-sm sm:text-base font-bold uppercase tracking-wide text-foreground leading-none">Portfolio Request Submitted</CardTitle>
 ...
 Your portfolio building request is now in queue.
 </CardDescription>
 </CardHeader>
 ...
 {generatedRequestId.slice(0, 8).toUpperCase()}E
 </p>
 </div>
 ...
 <p className="font-mono text-[10px] font-black uppercase tracking-wide text-primary">Free Promotion Active</p>
 <p className="font-mono text-[9px] font-bold text-muted-foreground/50 uppercase tracking-tight block truncate tabular-nums">
 Available: {remainingFreePromoSlots.toLocaleString()} free requests remaining
 </p>
 </div>
 </div>
 </CardContent>
 ...
 <span className="pt-0.5">Portfolio building fee</span>
 </div>
 ...
 {steps[currentStepIndexNum].label}
 </CardTitle>
 </div>
 ...
 Step {currentStepIndexNum + 1} of 5
 </span>
 </CardHeader>
 ...
 <Label className="font-mono text-[10px] font-extrabold uppercase text-muted-foreground/60 tracking-wide block leading-none ml-0.5">Full Name</Label>
 <Input
 type="text"
 ...
 <div className="space-y-1 block leading-none">
 <Label className="font-mono text-[10px] font-extrabold uppercase text-muted-foreground/60 tracking-wide block leading-none ml-0.5">Email Address</Label>
 <Input
 type="email"
 ...
 <div className="space-y-1 block leading-none">
 <Label className="font-mono text-[10px] font-extrabold uppercase text-muted-foreground/60 tracking-wide block leading-none ml-0.5">WhatsApp Number</Label>
 <Input
 type="tel"
 ...
 <div className="space-y-1 block leading-none">
 <Label className="font-mono text-[10px] font-extrabold uppercase text-muted-foreground/60 tracking-wide block leading-none ml-0.5">Professional Field / Category</Label>
 <Select
 ...
 >
 <FileUp className="h-3.5 w-3.5 mr-1.5 stroke-[2.2] shrink-0" /> <span>Upload File</span>
 </Button>
 <Button
 type="button"
 ...
 >
 <Globe className="h-3.5 w-3.5 mr-1.5 stroke-[2.2] shrink-0" /> <span>Paste URL</span>
 </Button>
 <Button
 type="button"
 ...
 >
 <PenLine className="h-3.5 w-3.5 mr-1.5 stroke-[2.2] shrink-0" /> <span>Fill Profile</span>
 </Button>
 </div>
 ...
 <Label className="font-mono text-[10px] font-extrabold uppercase text-muted-foreground/60 tracking-wide block leading-none ml-0.5">Resume URL</Label>
 <Input
 type="url"
 value={formDataState.cvExternalUrl}
 onChange={(e) => setFormDataState((prev) => ({ ...prev, cvExternalUrl: e.target.value }))}
 placeholder="e.g., https://linkedin.com/in/username/cv.pdf"
 ...
 description="Upload certificates, awards, or other credentials."
 />
 
 <div className="space-y-1 block leading-none pt-1">
 <Label className="font-mono text-[10px] font-extrabold uppercase text-primary tracking-wide block leading-none ml-0.5">Key Achievements</Label>
 <Textarea
 value={formDataState.achievements}
 onChange={(e) => setFormDataState((prev) => ({ ...prev, achievements: e.target.value }))}
 placeholder="Highlight your key career achievements and milestones..."
 ...
 <Label className="font-mono text-[10px] font-extrabold uppercase text-muted-foreground/60 tracking-wide block leading-none ml-0.5 capitalize">{fieldKeyStr} Profile URL</Label>
 <Input
 type="url"
 ...
 placeholder={`https://${fieldKeyStr}.com/username`}
 className="h-10 text-xs sm:text-sm bg-background/50 border border-border/60 focus-visible:ring-1 focus-visible:ring-ring rounded-lg shadow-none"
 />
 </div>
 ))}
 ...
 <div className="space-y-1 block leading-none pt-1">
 <Label className="font-mono text-[10px] font-extrabold uppercase text-primary tracking-wide block leading-none ml-0.5">Additional Notes for Designers</Label>
 <Textarea
 ...
 <span className="font-mono text-[9px] font-bold text-muted-foreground/40 uppercase tracking-wide select-none pointer-events-none">Full Name</span>
 <span className="text-foreground text-xs font-bold uppercase tracking-wide">{formDataState.fullName}</span>
 </div>
 <div className="flex justify-between items-center leading-none w-full block">
 <span className="font-mono text-[9px] font-bold text-muted-foreground/40 uppercase tracking-wide select-none pointer-events-none">Resume / CV</span>
 ...
 <div className="flex justify-between items-center leading-none w-full block">
 <span className="font-mono text-[9px] font-bold text-muted-foreground/40 uppercase tracking-wide select-none pointer-events-none">Category</span>
 <span className="text-foreground text-xs font-bold uppercase tracking-wide">{activeSelectedCategoryNode?.name || "UNASSIGNED"}</span>
 </div>
 ...
 <span className="font-mono text-[10px] font-black tracking-widest text-primary uppercase select-none pointer-events-none">Total Price</span>
 <span className="text-base font-mono font-black tracking-tight text-foreground uppercase tabular-nums">
 {isFreePromotionActive ? "Free (Promotion)" : `${PORTFOLIO_COST.toLocaleString()} Credits`}
 </span>
 </div>
 </div>
 ...
 Secure submission enabled. Your data is protected.
 </p>
 </div>
 </div>
 )}
 ...
 >
 <ArrowLeft className="mr-1 h-3.5 w-3.5 stroke-[2.5]" /> <span>Back</span>
 </Button>
 )}
 ...
 <span className="truncate block pt-0.5">Submitting Request...</span>
 </div>
 ) : (
 <div className="flex items-center justify-center gap-1.5 mx-auto leading-none">
