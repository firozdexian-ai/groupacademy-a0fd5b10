...
export function EventsTab() {
  ...
  const { data: events, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (eventType !== 'all') query = query.eq('content_type', eventType);
  ...
  if (isLoading) {
    return (
      <div className="grid gap-4 md:grid-cols-2">
        {[1, 2, 3, 4].map(i => (
          <Card key={i}><Skeleton className="aspect-video" /><CardHeader><Skeleton className="h-5 w-3/4" /><Skeleton className="h-4 w-full" /></CardHeader><CardContent><Skeleton className="h-4 w-1/2 mb-2" /><Skeleton className="h-4 w-1/3" /></CardContent></Card>
        ))}
      </div>
    );
  }
