...
import { Copy, Download, Loader2, Plus, MessageSquare } from "lucide-react";
...
export function StandaloneMockInterviewCodeGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success(`Generated ${codes.length} mock interview code(s)`);
    } catch (error: any) {
  ...
  const downloadCodes = () => {
    ...
    const blob = new Blob([`Mock Interview Access Codes for: ${email}\n\n${content}`], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `mock-interview-codes-${email}.txt`;
    a.click();
