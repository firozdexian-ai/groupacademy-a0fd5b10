import * as React from "react";
import * as HoverCardPrimitive from "@radix-ui/react-hover-card";
import { cn } from "@/lib/utils";

/**
 * GroUp Academy: Contextual Intel Disclosure Overlay Interface (HoverCard)
 * Hardened non-interruptive informational card isolating auxiliary preview data blocks with seamless viewport boundaries.
 * Version: Launch Candidate Â· Phase Z0 Geometric Balance Lock
 */
const HoverCard = HoverCardPrimitive.Root;

const HoverCardTrigger = HoverCardPrimitive.Trigger;

const HoverCardContent = React.forwardRef<
  React.ElementRef<typeof HoverCardPrimitive.Content>,
  React.ComponentPropsWithoutRef<typeof HoverCardPrimitive.Content>
>(({ className, align = "center", sideOffset = 6, ...props }, ref) => (
  <HoverCardPrimitive.Content
    ref={ref}
    align={align}
    sideOffset={sideOffset}
    className={cn(
      "z-50 w-72 sm:w-80 rounded-xl border border-border/40 bg-popover/95 backdrop-blur-md p-4 sm:p-5 text-popover-foreground shadow-md outline-none text-left antialiased select-none sm:select-text transform-gpu",
      "data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-top-1.5",
      "data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=closed]:slide-out-to-top-1.5",
      className,
    )}
    {...props}
  />
));

HoverCardContent.displayName = "HoverCard_Core_Content_Node";

export { HoverCard, HoverCardTrigger, HoverCardContent };

